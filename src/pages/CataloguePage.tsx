...
export function CataloguePage() {
  ...
  return (
    <div className="bg-[#F8F9FA] text-[#0D0D0F] min-h-screen antialiased">
      {/* ─── 1. CLEAN CATALOGUE HERO (LIGHT / OFF-WHITE THEME) ─── */}
      <section className="relative overflow-hidden bg-white border-b border-[#E4E4E7] py-12 sm:py-16">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 relative z-10 text-center max-w-3xl">
          <Reveal>
            <span className="inline-flex items-center gap-2 rounded-full border border-[#9B1B9E]/30 bg-[#9B1B9E]/10 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.2em] text-[#9B1B9E] mb-3">
              <FileText className="h-3.5 w-3.5" />
              PRODUCT CATALOGUE
            </span>

            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0D0D0F] tracking-tight leading-tight">
              Explore Our <span className="text-[#9B1B9E]">Catalogue</span>
            </h1>

            <p className="mt-3.5 text-xs sm:text-sm text-[#52525B] leading-relaxed font-medium">
      ...
      <section className="sticky top-[56px] sm:top-[64px] z-40 bg-white/95 backdrop-blur-md border-b border-[#E4E4E7] py-3.5 shadow-sm">
            ...
            {catalogueCategories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 rounded-full px-4.5 py-2 text-xs font-bold transition-all duration-200 cursor-pointer ${
                    isActive
            ...
            <AnimatePresence mode="popLayout">
              {filteredDocuments.map((doc, idx) => (
                <motion.div
                  ...
                  className="group flex flex-col justify-between overflow-hidden rounded-[22px] border border-[#E4E4E7] bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#9B1B9E] hover:shadow-xl h-full"
                >
                  <div>
                    {/* Brochure Image Preview Box — FIT TO FRAME */}
                    <div className="relative aspect-[3/4] sm:aspect-[4/5] w-full overflow-hidden rounded-xl bg-[#F8F9FA] border border-[#E4E4E7] p-2 flex items-center justify-center mb-4 group-hover:bg-slate-50 transition-colors">
                      <img
                        src={doc.image}
                        alt={doc.title}
                        loading="lazy"
                      ...
                      {/* PDF Tag Overlay */}
                      <div className="absolute top-3 left-3 rounded-full bg-white/90 border border-[#E4E4E7] px-2.5 py-1 text-[10px] font-bold text-[#9B1B9E] backdrop-blur flex items-center gap-1 shadow-sm">
                        <FileText className="h-3 w-3 text-[#FF6B00]" />
                        <span>{doc.fileSize}</span>
                      </div>

                      <div className="absolute top-3 right-3 rounded-full bg-slate-900/80 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur">
                        {doc.pages}
                      </div>
                    </div>

                    {/* Category Label */}
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#9B1B9E] block mb-1">
                      {doc.category}
                    </span>
                  ...
                  <div className="mt-5 pt-3.5 border-t border-[#E4E4E7] flex items-center justify-between">
                    <button
                      ...
                      className="w-full inline-flex items-center justify-between rounded-xl bg-[#9B1B9E] px-4 py-2.5 text-xs font-bold text-white shadow-sm transition-all duration-200 hover:bg-[#B52CB8] group/btn cursor-pointer"
                    >
