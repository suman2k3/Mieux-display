...
export function EventGalleryCard({ event, index, onOpenLightbox }: EventGalleryCardProps) {
  ...
  const renderImage = (img: GalleryPhoto, idx: number, heightClass: string, isLightSection: boolean) => (
    <motion.div
      key={img.id}
      whileHover="hover"
      className={`group relative cursor-pointer overflow-hidden rounded-[20px] shadow-sm transition-all duration-300 hover:shadow-xl ${
        isLightSection
    ...
    >
      ...
      <div className="absolute bottom-3 right-3 z-10 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 translate-x-2">
        <div className="inline-flex items-center gap-1.5 rounded-full bg-[#050505]/85 border border-[#9B1B9E]/50 px-3 py-1 text-[11px] font-bold text-white backdrop-blur-md shadow-lg">
      ...
      return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 items-stretch">
          <div className="lg:col-span-7">
            {renderImage(firstImage, 0, "h-[320px] sm:h-[400px] lg:h-[440px]", isLight)}
          </div>
          <div className="lg:col-span-5 grid grid-cols-2 gap-4">
            {supporting.map((img, idx) =>
              renderImage(img, idx + 1, "h-[152px] sm:h-[192px] lg:h-[212px]", isLight)
            )}
          </div>
        </div>
      );
      ...
      return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 items-stretch">
          ...
          <div className="lg:col-span-7 order-1 lg:order-2">
            {renderImage(mainImage, 0, "h-[320px] sm:h-[400px] lg:h-[440px]", isLight)}
          </div>
        </div>
      );
    }
    ...
    if (layoutType === 2) {
      ...
      return (
        <div className="space-y-4">
          <div>{renderImage(mainImage, 0, "h-[300px] sm:h-[380px] lg:h-[420px]", isLight)}</div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {supporting.map((img, idx) =>
              renderImage(img, idx + 1, "h-[180px] sm:h-[210px] lg:h-[230px]", isLight)
            )}
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {topTwo.map((img, idx) =>
            renderImage(img, idx, "h-[260px] sm:h-[320px] lg:h-[350px]", isLight)
          )}
        </div>
        {bottomRow.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {bottomRow.map((img, idx) =>
              renderImage(img, idx + 2, "h-[180px] sm:h-[200px] lg:h-[220px]", isLight)
            )}
          </div>
        )}
      </div>
  ...
  return (
    <section
      id={event.slug}
      className={`py-14 sm:py-20 lg:py-24 border-b ${
        isDark
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span
              className={`inline-flex items-center rounded-full px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider ${
                isDark
        ...
        <Reveal delay={0.15}>
          <div className="mt-8 flex justify-center sm:justify-end">
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-full bg-[#9B1B9E] px-6 py-3 text-xs font-bold text-white shadow-md transition-all hover:bg-[#B52CB8] hover:-translate-y-0.5"
            >
