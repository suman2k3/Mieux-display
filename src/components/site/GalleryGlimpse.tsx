...
export function GalleryGlimpse() {
  ...
  return (
    <section className="py-16 lg:py-24 bg-[#050505] text-[#F5F5F5] border-b border-[#27272A]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 lg:mb-12 gap-6">
          ...
          <Reveal delay={0.2}>
            <Link
              to="/gallery"
              className="inline-flex items-center text-[#9B1B9E] hover:text-white font-bold transition-colors text-xs uppercase tracking-wider"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5 md:gap-6">
          <Reveal>
            <div className="relative aspect-[16/10] lg:aspect-auto lg:h-full min-h-[360px] lg:min-h-[440px] rounded-[22px] overflow-hidden group shadow-2xl border border-[#27272A] bg-[#151518]">
              <img
                src={featureImage.url}
                alt={featureImage.alt || "Event feature image"}
                className="w-full h-full object-cover opacity-100 transition-transform duration-500 group-hover:scale-[1.02]"
                loading="lazy"
              />
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-5">
            {supportingImages.map((imgItem, idx) => (
              <Reveal key={idx} delay={0.1 * (idx + 1)}>
                <div className="relative aspect-square rounded-[20px] overflow-hidden group shadow-2xl border border-[#27272A] bg-[#151518]">
                  <img
                    src={imgItem.url}
